...
## 作業ログ (2026/04/10 - 2026/04/27)

### ■ 内容
- NexusFlowのコンセプト整理（タスク管理 × 意思決定）
- 4象限ベースの構想からスコアリング中心へ方針転換
- 主観（やりたい）と客観（やるべき）を組み合わせる設計に整理
- スコアリングロジックの検討開始（ver0.1を仮定義）
- READMEを改善（意思決定アプリとしての位置付けを明確化）
- docsにscoring-logを新規作成

### ■ 意図
- 「分類ではなく意思決定」という差別化の明確化
- 優先順位決定の再現性を持たせる設計へのシフト

### ■ 判断
- スコアリングはコアロジックのため即確定せず、検証前提とする
- spec.yamlは未変更（確定前のため）

### ■ メモ
- 設計思想は固まったが、アルゴリズムは未確定
- 今後はver0.1をベースに改善予定
> ※ 思考ログとして記録（正確な時系列ではなく変化の要点を優先）